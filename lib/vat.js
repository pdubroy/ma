...
//    this.step();
  ...
  _tryWatch(pattern, cb) {
    if (pattern instanceof combine) {
      ...
      var firstMatches = Immutable.List(matchIters.map(it => Immutable.Seq(it).first()));
      ...
      if (firstMatches.every(m => m !== undefined)) {
        if (firstMatches.groupBy(m => m.index).size !== firstMatches.size) {
