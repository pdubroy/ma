...
class Vat extends EventEmitter {
  ...
  *_getMatches(pattern, store=this._store) {
    ...
    for (var [key, obj] of store.entries()) {
  ...
  *_getDeepMatches(pattern) {
    var p = convertPattern(pattern);
    var path;
    for (var [key, obj] of this._store.entries()) {
      path = [key];
      var root = obj.value;
  ...
  _collectReactionCandidates(...lists) {
    ...
    [].concat(...lists).forEach((r) => {
      if (r instanceof MultiReaction) {
        ...
        var matches = Immutable.Seq(this._getDeepMatches(r.pattern)).filter(accept);
        candidates = candidates.mergeWith(
            (prev, next) => prev.concat(next),
            matches.map(m => [r, m]).groupBy(arr => arr[1].index));
      }
    });
    return candidates;
  }
