...
class Vat extends EventEmitter {
  ...
  *_getMatches(pattern, store=this._store) {
    ...
    for (var i = 0; i < store.size; ++i) {
      var bindings = match(store.get(i).value, p);
      if (bindings) {
        yield [i, bindings];
      }
    }
  ...
  *_getDeepMatches(pattern) {
    var p = convertPattern(pattern);
    var path;
    for (var i = 0; i < this._store.size; ++i) {
      path = [i];
      var root = this._store.get(i).value;
  ...
  _collectReactionCandidates(...lists) {
    ...
    [].concat(...lists).forEach((r) => {
      if (r instanceof MultiReaction) {
        ...
        var matches = gu.filter(this._getDeepMatches(r.pattern), accept);
        matches.forEach(m => {
          var i = m.index;
